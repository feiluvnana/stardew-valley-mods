# 🗺️ BetterMap for Stardew Valley 1.6+

A high-performance map enhancement and quality-of-life mod for **Stardew Valley 1.6+**, built with SMAPI.

---

## ✨ Features

- **🚪 3x1 Farmhouse Exit Doorway**: Widens the exit doorway to 3 tiles wide across all Farmhouse upgrade levels (`FarmHouse`, `FarmHouse1`, `FarmHouse2`) and the Ginger Island Farmhouse (`IslandFarmHouse`), allowing comfortable, seamless 3x1 exiting without being bottlenecked into a 1x1 tile.
- **🪵 Remove Farm Driftwood Fence & Logs**: Completely removes the long horizontal driftwood fence and bleached log piles across Ginger Island Farm (Island West) that separate the upper grassy path from the farmable fields, creating a wide, seamless open farm.
- **⚙️ Full GMCM Support**: In-game configuration menu support with real-time map cache updating when settings change.
- **🌐 Localization**: Full English and Vietnamese support.

---

## ⚙️ Configuration (Generic Mod Config Menu)

| Setting | Default | Description |
| :--- | :---: | :--- |
| `WidenHouseExit` | `true` | Widens the exit doorway of all Farmhouses to 3x1. |
| `RemoveFarmDriftwoodBarrier` | `true` | Removes the driftwood fence and bleached log piles across the Ginger Island Farm. |

---

## 🛠️ Building the Mod

```powershell
dotnet build BetterMap.csproj
```

---

## 📄 License
Created by **feiluvnana** for Stardew Valley 1.6+. Built with SMAPI.
